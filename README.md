# Massa Project Install and Update scripts (valid for 24.x episode)
...
rm -rf ~/massa
```
## UPDATE the node (any version 24.x):
1. Load the script.
```
sudo apt update && sudo apt install curl -y && curl -sLO https://raw.githubusercontent.com/avkarcr/massa_project/main/massa_24_update.sh && chmod +x massa_24_update.sh
```
2. Run the script.
```
./massa_24_update.sh
...
## INSTALL the node (24.0):
1. You need a new server with Ubuntu 20+ on server to install the node
2. Load the script for further installation.
```
sudo apt update && sudo apt install curl -y && curl -sLO https://raw.githubusercontent.com/avkarcr/massa_project/main/massa_24_install.sh && chmod +x massa_24_install.sh
```
...
# [РУССКИЙ ЯЗЫК] Установка и обновление ноды Massa версии 24.х
...
rm -rf ~/massa
```
## Инструкция по ОБНОВЛЕНИЮ (для любой подверсии 24.x):
1. Скачиваем скрипт.
```
sudo apt update && sudo apt install curl -y && curl -sLO https://raw.githubusercontent.com/avkarcr/massa_project/main/massa_24_update.sh && chmod +x massa_24_update.sh
```
